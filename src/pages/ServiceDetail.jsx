import { useState } from 'react';
import { useParams, useNavigate, Link } from 'react-router-dom';
import { ArrowLeft, Star, Clock, Shield, Check, ChevronDown, ChevronUp, Plus, ArrowRight } from 'lucide-react';
import { SERVICES } from '@/lib/services';
import { Button } from '@/components/ui/button';

const PACKAGES = [
  { name: 'Basic', desc: 'Standard clean', multiplier: 1, includes: ['Sweeping & mopping', 'Bathroom cleaning', 'Dusting surfaces', 'Trash removal'] },
  { name: 'Premium', desc: 'Deep clean + inspection', multiplier: 1.6, includes: ['Everything in Basic', 'Kitchen deep clean', 'Window wiping', 'Appliance exterior'] },
  { name: 'Complete', desc: 'Full service + warranty', multiplier: 2.2, includes: ['Everything in Premium', 'Refrigerator inside', 'Oven cleaning', '60-day rework guarantee'] },
];

const ADDONS = [
  { id: 'extra_bathroom', label: 'Extra Bathroom', price: 25 },
  { id: 'window_cleaning', label: 'Window Cleaning', price: 40 },
  { id: 'carpet_vacuum', label: 'Carpet Vacuum', price: 30 },
  { id: 'ironing', label: 'Ironing (up to 20 items)', price: 35 },
];

const FAQS = [
  { q: 'What should I do before the team arrives?', a: 'Clear walkways and secure valuables. No need to provide cleaning supplies — we bring everything.' },
  { q: 'Are your professionals insured?', a: 'Yes, all FixMate partners are fully insured and bonded. Any accidental damage is covered.' },
  { q: 'Can I request the same partner again?', a: 'Absolutely! After your first booking, you can set a partner as your favourite and request them directly.' },
  { q: 'What if I am not satisfied?', a: 'We offer a free rework within 48 hours, no questions asked. Customer satisfaction is our priority.' },
];

const REVIEWS = [
  { name: 'Nurul Ain', rating: 5, comment: 'Aisyah was amazing! Very thorough and professional. My apartment is spotless.', date: '2 days ago' },
  { name: 'David Lim', rating: 5, comment: 'Punctual, efficient, and friendly. Will definitely book again.', date: '1 week ago' },
  { name: 'Siti Rahimah', rating: 4, comment: 'Good service overall. Bathroom could have been a bit more thorough.', date: '2 weeks ago' },
];

export default function ServiceDetail() {
  const { serviceId } = useParams();
  const navigate = useNavigate();
  const service = SERVICES.find(s => s.id === serviceId);
  const [pkgIdx, setPkgIdx] = useState(0);
  const [addons, setAddons] = useState([]);
  const [openFaq, setOpenFaq] = useState(null);

  if (!service) return <div className="flex justify-center pt-32"><div className="w-6 h-6 border-2 border-muted border-t-primary rounded-full animate-spin" /></div>;

  const Icon = service.icon;
  const basePrice = service.priceRange[0];
  const pkgPrice = Math.round(basePrice * PACKAGES[pkgIdx].multiplier);
  const addonTotal = addons.reduce((sum, id) => sum + (ADDONS.find(a => a.id === id)?.price || 0), 0);
  const totalPrice = pkgPrice + addonTotal;

  const toggleAddon = (id) => setAddons(prev => prev.includes(id) ? prev.filter(a => a !== id) : [...prev, id]);

  return (
    <div className="font-inter pb-32">
      {/* Hero Image */}
      <div className="relative h-64">
        <img src={service.image} alt={service.name} className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent" />
        <button onClick={() => navigate(-1)}
          className="absolute top-12 left-4 w-10 h-10 bg-white/95 backdrop-blur rounded-2xl flex items-center justify-center shadow-card">
          <ArrowLeft className="h-4 w-4" />
        </button>
        <div className="absolute bottom-0 left-0 right-0 px-5 pb-5">
          <div className="flex items-end justify-between">
            <div>
              <h1 className="text-white text-2xl font-bold tracking-tight">{service.name}</h1>
              <p className="text-white/70 text-sm mt-0.5">{service.description}</p>
            </div>
            <div className={`w-12 h-12 rounded-2xl ${service.color} flex items-center justify-center shadow-lg`}>
              <Icon className="h-6 w-6" />
            </div>
          </div>
        </div>
      </div>

      <div className="px-5 pt-5 space-y-7">

        {/* Stats Bar */}
        <div className="flex items-center gap-4 py-3 px-4 bg-white rounded-2xl border border-border shadow-card">
          <div className="flex items-center gap-1.5">
            <Star className="h-4 w-4 text-amber-400 fill-amber-400" />
            <span className="font-bold text-sm">4.8</span>
            <span className="text-xs text-muted-foreground">(2.4k reviews)</span>
          </div>
          <div className="w-px h-5 bg-border" />
          <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
            <Clock className="h-3.5 w-3.5" />{service.duration}
          </div>
          <div className="w-px h-5 bg-border" />
          <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
            <Shield className="h-3.5 w-3.5 text-primary" />
            <span className="text-xs font-medium text-primary">Insured</span>
          </div>
        </div>

        {/* Package Selector */}
        <div>
          <h3 className="text-base font-bold mb-3">Choose Package</h3>
          <div className="space-y-2.5">
            {PACKAGES.map((pkg, i) => {
              const price = Math.round(basePrice * pkg.multiplier);
              const active = pkgIdx === i;
              return (
                <button key={i} onClick={() => setPkgIdx(i)}
                  className={`w-full text-left rounded-2xl border-2 p-4 transition-all duration-200 ${active ? 'border-primary bg-accent shadow-card' : 'border-border bg-white shadow-sm hover:border-primary/40'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${active ? 'border-primary bg-primary' : 'border-border'}`}>
                        {active && <Check className="h-3 w-3 text-white" />}
                      </div>
                      <span className="font-bold text-sm">{pkg.name}</span>
                      {i === 1 && <span className="text-[9px] bg-primary text-white px-2 py-0.5 rounded-full font-bold">POPULAR</span>}
                    </div>
                    <span className="font-bold text-primary">RM{price}</span>
                  </div>
                  <p className="text-xs text-muted-foreground mb-2 pl-7">{pkg.desc}</p>
                  {active && (
                    <ul className="pl-7 space-y-1">
                      {pkg.includes.map((item, j) => (
                        <li key={j} className="flex items-center gap-1.5 text-xs text-foreground">
                          <Check className="h-3 w-3 text-primary shrink-0" />{item}
                        </li>
                      ))}
                    </ul>
                  )}
                </button>
              );
            })}
          </div>
        </div>

        {/* Add-ons */}
        <div>
          <h3 className="text-base font-bold mb-3">Add-ons</h3>
          <div className="grid grid-cols-2 gap-2">
            {ADDONS.map(addon => {
              const active = addons.includes(addon.id);
              return (
                <button key={addon.id} onClick={() => toggleAddon(addon.id)}
                  className={`flex flex-col items-start gap-1 p-3 rounded-2xl border-2 text-left transition-all ${active ? 'border-primary bg-accent' : 'border-border bg-white'}`}>
                  <div className="flex items-center justify-between w-full">
                    <span className="text-xs font-semibold">{addon.label}</span>
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center transition-all ${active ? 'bg-primary' : 'bg-muted'}`}>
                      {active ? <Check className="h-3 w-3 text-white" /> : <Plus className="h-3 w-3 text-muted-foreground" />}
                    </div>
                  </div>
                  <span className="text-xs font-bold text-primary">+RM{addon.price}</span>
                </button>
              );
            })}
          </div>
        </div>

        {/* Reviews */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-base font-bold">Reviews</h3>
            <div className="flex items-center gap-1">
              <Star className="h-4 w-4 text-amber-400 fill-amber-400" />
              <span className="font-bold text-sm">4.8</span>
            </div>
          </div>
          <div className="space-y-3">
            {REVIEWS.map((r, i) => (
              <div key={i} className="bg-white rounded-2xl border border-border p-4 shadow-card">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-xl bg-primary/10 flex items-center justify-center">
                      <span className="text-xs font-bold text-primary">{r.name.charAt(0)}</span>
                    </div>
                    <p className="font-semibold text-sm">{r.name}</p>
                  </div>
                  <div className="flex items-center gap-0.5">
                    {[...Array(r.rating)].map((_, j) => (
                      <Star key={j} className="h-3 w-3 text-amber-400 fill-amber-400" />
                    ))}
                  </div>
                </div>
                <p className="text-xs text-muted-foreground leading-relaxed">{r.comment}</p>
                <p className="text-[10px] text-muted-foreground mt-2">{r.date}</p>
              </div>
            ))}
          </div>
        </div>

        {/* FAQ */}
        <div>
          <h3 className="text-base font-bold mb-3">FAQs</h3>
          <div className="space-y-2">
            {FAQS.map((faq, i) => (
              <div key={i} className="bg-white rounded-2xl border border-border overflow-hidden shadow-card">
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  className="w-full flex items-center justify-between px-4 py-3.5 text-left">
                  <span className="text-sm font-medium pr-3">{faq.q}</span>
                  {openFaq === i ? <ChevronUp className="h-4 w-4 text-primary shrink-0" /> : <ChevronDown className="h-4 w-4 text-muted-foreground shrink-0" />}
                </button>
                {openFaq === i && (
                  <div className="px-4 pb-4 text-xs text-muted-foreground leading-relaxed border-t border-border/50 pt-3">
                    {faq.a}
                  </div>
                )}
              </div>
            ))}
          </div>
        </div>
      </div>

      {/* Sticky Booking Bar */}
      <div className="fixed bottom-0 left-0 right-0 z-50">
        <div className="max-w-lg mx-auto bg-white/95 backdrop-blur-xl border-t border-border px-5 py-4">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-xs text-muted-foreground">{PACKAGES[pkgIdx].name} package{addons.length > 0 ? ` + ${addons.length} add-on${addons.length > 1 ? 's' : ''}` : ''}</p>
              <div className="flex items-baseline gap-1">
                <span className="text-xl font-bold text-primary">RM{totalPrice}</span>
                <span className="text-xs text-muted-foreground">total</span>
              </div>
            </div>
            <Button asChild className="rounded-2xl px-6 h-12 shadow-float">
              <Link to={`/book/${service.id}?package=${pkgIdx}&addons=${addons.join(',')}`}>
                Book Now <ArrowRight className="h-4 w-4 ml-1" />
              </Link>
            </Button>
          </div>
        </div>
      </div>
    </div>
  );
}